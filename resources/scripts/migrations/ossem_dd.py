...
def event_code_to_event_id(data_dictionary):
    if data_dictionary and "event_code" in data_dictionary:
        ...
        (position, value) = next((i, x[1]) for (i, x) in enumerate(
            data_dictionary_items) if x[0] == "event_code")
...
def title_to_name(data_dictionary):
    if data_dictionary and "title" in data_dictionary:
        ...
        data_dictionary_items = list(data_dictionary.items())
        (position, value) = next((i, x[1]) for (i, x) in enumerate(
            data_dictionary_items) if x[0] == "title")
...
def standard_name_from_camel_to_snake(data_dictionary):
    ...
    if data_dictionary and "event_fields" in data_dictionary:
        for field in data_dictionary['event_fields']:
            if "standard_name" in field:
                field['standard_name'] = to_snake_case(field['standard_name'])
    return data_dictionary


def standard_name_from_snake_to_camel(data_dictionary):
    def to_camel_case(snake_str):
        return ''.join([c[0].upper() + c[1:]if len(c) > 0 else '' for c in snake_str.split('_')])

    if data_dictionary and "event_fields" in data_dictionary:
        for field in data_dictionary['event_fields']:
            if "standard_name" in field and field != "TBD":
                field['standard_name'] = to_camel_case(field['standard_name'])
    return data_dictionary
    ...
    **Parameters**:
    ...
    for file_or_dir in os.listdir(path):
        ...
        if os.path.isdir(file_or_dir_path):
            ...
            to_process.extend(crawl_data_dictionaries(
                file_or_dir_path, ignored_file_or_dir))
